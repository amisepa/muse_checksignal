...
outDir = fullfile(mainDir, 'outputs','features');
...
segSize = segSize * 256;            % convert to samples
...
for iSeg = 1:nSeg-1
    ...
    tmp = get_psd(front_bad(tSeg),256,'hamming',50,[],256,[70 100],'psd');
    front_bad_HF(iSeg,:) = rms(tmp);
    tmp = get_psd(post_good(tSeg),256,'hamming',50,[],256,[70 100],'psd');
    post_good_HF(iSeg,:) = rms(tmp);
    tmp = get_psd(post_bad(tSeg),256,'hamming',50,[],256,[70 100],'psd');
    post_bad_HF(iSeg,:) = rms(tmp);

    % Compute rms of LF power
    tmp = get_psd(front_good(tSeg),256,'hamming',50,[],256,[0.001 3],'psd');
    front_good_LF(iSeg,:) = rms(tmp);
    tmp = get_psd(front_bad(tSeg),256,'hamming',50,[],256,[0.001 3],'psd');
    front_bad_LF(iSeg,:) = rms(tmp);
    tmp = get_psd(post_good(tSeg),256,'hamming',50,[],256,[0.001 3],'psd');
    post_good_LF(iSeg,:) = rms(tmp);
    tmp = get_psd(post_bad(tSeg),256,'hamming',50,[],256,[0.001 3],'psd');
...
% save(fullfile(outDir, 'front_good_KURT'), 'front_good_KURT')
...
var_front = { 'front_good_KURT' 'front_bad_KURT' 'front_good_RMS' 'front_bad_RMS' ...
    ...
    'front_good_HF' 'front_bad_HF' 'front_good_LF' 'front_bad_LF' ...
    'front_good_SAMP' 'front_bad_SAMP' 'front_good_APP' 'front_bad_APP' ...
    'front_good_FUZ' 'front_bad_FUZ' };
var_post = { 'post_good_KURT' 'post_bad_KURT' 'post_good_RMS' 'post_bad_RMS'...
    ...
    'post_good_HF' 'post_bad_HF' 'post_good_LF' 'post_bad_LF' ...
    'post_good_SAMP' 'post_bad_SAMP' 'post_good_APP' 'post_bad_APP'...
    'post_good_FUZ' 'post_bad_FUZ' };
